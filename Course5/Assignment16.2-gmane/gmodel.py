...
#cleans up the data from the db, updates are made since data too old, the new index.sqlite it's much smaller. normalize
...
def fixsender(sender,allsenders=None) :                  #org and people change address from time to time
    global dnsmapping
    global mapping
    ...
    if allsenders is not None and sender.endswith('gmane.org') : #gmane.org is when people want to hide their email address.
        pieces = sender.split('-')
        realsender = None
        for s in allsenders:
...
for message_row in cur_1 :
    old = fixsender(message_row[0])         #email address from gmane is kind of funky
...
conn_1 = sqlite3.connect('file:content.sqlite?mode=ro', uri=True) #running both this and spider at the same time
cur_1 = conn_1.cursor()
...
for message_row in cur_1 :
    ...
    allsenders.append(sender)               #take all the email address and put them in a list
...
for message_row in cur_1 :
    ...
    (guid, sender, subject, sent_at) = parsed         #parse the headers

    # Apply the sender mapping
    sender = mapping.get(sender,sender)          #look up sender if it's in there give me the entry of that key, otherwise give me og sender back
    ...
    sender_id = senders.get(sender,None)  #get the mapping between the senders which is a way to lookup the primary key make them cached than in db can make the process faster
    ...
    if sender_id is None :
        ...
        try:
            row = cur.fetchone()
            sender_id = row[0]
            senders[sender] = sender_id #build senders dictionary up
        except:
    ...
    cur.execute('INSERT OR IGNORE INTO Messages (guid,sender_id,subject_id,sent_at,headers,body) VALUES ( ?,?,?,datetime(?),?,? )',
            ...
            zlib.compress(message_row[0].encode()), zlib.compress(message_row[1].encode())) ) #zlib compress: taking the message, header and the body and compress them, take the computation of the body to compress and decompress when read it
    conn.commit()
