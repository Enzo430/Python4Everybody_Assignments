...
cur.execute('SELECT id, sender FROM Senders') #reads index.sqlite make a dictionary of id to senders, make things faster, can also use join, make it cleaner
senders = dict()
for message_row in cur :
    senders[message_row[0]] = message_row[1] #senders[id]= sender
...
cur.execute('SELECT id, guid,sender_id,subject_id,sent_at FROM Messages') #not selecting body or headers here
messages = dict()
for message_row in cur :
...
for (message_id, message) in list(messages.items()):
    ...
    pieces = senders[sender].split("@")  #person and domain
